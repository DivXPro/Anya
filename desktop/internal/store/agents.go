...
func UpdateAgentSelected(db *sql.DB, id string, selected bool) error {
	_, err := db.Exec("UPDATE agents SET selected = ? WHERE id = ?", boolToInt(selected), id)
	return err
}
...
func ClearAgentSelection(db *sql.DB) error {
	_, err := db.Exec("UPDATE agents SET selected = 0")
	return err
}
